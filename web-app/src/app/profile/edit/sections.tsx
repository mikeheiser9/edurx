      ...
      <div className="my-4">
        ...
        <div className="grid grid-cols-2 gap-6 mt-2">
          {Object.keys(socialMediaIcons).map((socialMedia: string) => (
            <div className="flex gap-4 flex-row" key={socialMedia}>
              ...
              <InputField
                iconContainerClass="absolute text-white mx-2 mt-2"
                icon={
                  <span className="font-semibold" aria-disabled={true}>
                    /
                  </span>
                }
      ...
      {userData?.educations?.map((value: education) => (
        <div className="flex gap-x-6" key={value?._id || Date.now()}>
          ...
          <div className="flex flex-1 flex-col text-white gap-1">
            ...
            <span className="text-muted text-xs">{value.field_of_study}</span>
            <div>
...
const UserDocs = ({
  ...
  let dataKey = currentSection === "certifications" ? "certificate" : "license";
  ...
  const ListView = ({ dataKey }: { dataKey: string }) => (
    <>
...
const ProfileImages = ({
  ...
  const DropZone = ({ img_type }: { img_type: keyof profileImages }) => {
    ...
    const removeImage = () => {
      ...
      actions.setFieldValue(img_type, null);
              ...
              <div className="flex items-center justify-center">
                <Image
                  className={`object-cover w-full h-fit max-h-fit transition-all duration-300 ease-in-out ${
                    isHovered ? "brightness-50 blur-sm" : ""
                  }`}
                  height={800}
                  width={800}
                  alt={img_type}
